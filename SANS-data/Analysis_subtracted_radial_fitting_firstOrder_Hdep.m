...
starting_point = [500  1.4  0  0.011  1.8e-3];
upper_limit =    [Inf  Inf  1  12e-3  6.0e-3];
lower_limit =    [0     0  -1  06e-3  1.5e-3];
...
for currentFileNumber = 1:number_of_files
    ...
    if currentFileNumber == 5
        opts.StartPoint = [50.5 2e3  0  0.0750 3.0e-3];
...
plot(field, fitting_results(:, 3), 'Marker', 'diamond', 'LineStyle','-', 'Color', 'green');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 5), 'Marker', 'o', 'LineStyle','-', 'Color', 'blue');
xlabel('H field (mT)');
