...
fitting_results = zeros(number_of_files, 18);
...
lower_limit =    [0      0    0    0    -1   06e-3  10e-3  14e-3  1.0e-3  1.0e-3  1.0e-3];

excluded_angle1 = -1000; %045; % set to -100 if no region is excluded
excluded_angle2 = -1000; %135; % set to -100 if no region is excluded
excluded_angle3 = -1000; %225; % set to -100 if no region is excluded
excluded_angle4 = -1000; %315; % set to -100 if no region is excluded
dangle = 10; % excluded-angles half width in degrees

% cut off field
cut_temp = 1000; % I have not found a good H-field cut off value;
% stores the temperatures of each measurement
field = zeros(number_of_files, 1);
% stores the q bins
...
for currentFileNumber = 1:number_of_files
    ...
    field(currentFileNumber, 1) = str2double(strrep(strtok(fliplr(strtok(fliplr(currentFileName),'_')),'mT'),',','.'));
    ...
    if currentFileNumber == 20
        opts.StartPoint = [100   20   30   1000   0   0.0075 0.010  0.014  4.2e-3  2.2e-3  3.8e-3];
    ...
    %% Fit model to data.
    fitresult = fit(q_bins(14:80), int_bins(14:80, currentFileNumber), ft, opts);
    coefficients = coeffvalues(fitresult);
    
    if field(currentFileNumber) < cut_temp
    ...
    starting_point = 1.2*coefficients;
end
...
plot(field, fitting_results(:, 1), 'Marker', 'square', 'LineStyle','-', 'Color', 'red');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 3), 'Marker', 'square', 'LineStyle','-', 'Color', 'red');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 5), 'Marker', 'square', 'LineStyle','-', 'Color', 'red');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 7), 'Marker', 'diamond', 'LineStyle','-', 'Color', 'green');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 9), 'Marker', 'diamond', 'LineStyle','-', 'Color', 'green');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 11), 'Marker', 'diamond', 'LineStyle','-', 'Color', 'green');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 13), 'Marker', 'o', 'LineStyle','-', 'Color', 'blue');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 15), 'Marker', 'o', 'LineStyle','-', 'Color', 'blue');
xlabel('H field (mT)');
...
plot(field, fitting_results(:, 17), 'Marker', 'o', 'LineStyle','-', 'Color', 'blue');
xlabel('H field (mT)');
